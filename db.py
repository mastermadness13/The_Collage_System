...
def ensure_schema(conn=None):
    ...
    if 'courses' in existing_tables:
        ...
        if 'theoretical_hours' not in course_columns:
            conn.execute('ALTER TABLE courses ADD COLUMN theoretical_hours INTEGER NOT NULL DEFAULT 0')
        if 'practical_hours' not in course_columns:
            conn.execute('ALTER TABLE courses ADD COLUMN practical_hours INTEGER NOT NULL DEFAULT 0')
        if 'total_hours' not in course_columns:
            conn.execute('ALTER TABLE courses ADD COLUMN total_hours INTEGER NOT NULL DEFAULT 0')
        if 'accreditation' not in course_columns:
            conn.execute('ALTER TABLE courses ADD COLUMN accreditation TEXT')
        if 'vocabulary' not in course_columns:
            conn.execute('ALTER TABLE courses ADD COLUMN vocabulary TEXT')
    ...
    if 'teachers' in existing_tables:
        ...
        if 'academic_number' not in teacher_columns:
            conn.execute('ALTER TABLE teachers ADD COLUMN academic_number TEXT')
        if 'national_id' not in teacher_columns:
            conn.execute('ALTER TABLE teachers ADD COLUMN national_id TEXT')
        if 'qualification' not in teacher_columns:
            conn.execute('ALTER TABLE teachers ADD COLUMN qualification TEXT')
        if 'academic_rank' not in teacher_columns:
            conn.execute('ALTER TABLE teachers ADD COLUMN academic_rank TEXT')
        if 'classification' not in teacher_columns:
            conn.execute('ALTER TABLE teachers ADD COLUMN classification TEXT')
        if 'phone' not in teacher_columns:
            conn.execute('ALTER TABLE teachers ADD COLUMN phone TEXT')
        if 'contract_date' not in teacher_columns:
            conn.execute('ALTER TABLE teachers ADD COLUMN contract_date TEXT')
        if 'tasks' not in teacher_columns:
            conn.execute('ALTER TABLE teachers ADD COLUMN tasks TEXT')
    ...
    if 'rooms' in existing_tables:
        ...
        if 'building' not in room_columns:
            conn.execute('ALTER TABLE rooms ADD COLUMN building TEXT DEFAULT ""')
...
def create_default_departments():
    ...
    if default_department is not None:
        db.execute(
            ...
            WHERE role = 'head_of_department' AND (department_id IS NULL OR department_id = '')
            ''',
